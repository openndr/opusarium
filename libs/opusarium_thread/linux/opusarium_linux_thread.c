...
void opusarium_thread_init(opusarium_thread_t *thread)
{
    if (opusarium_likely(thread)) {
        ...
        opusarium_cpu_mask_empty(&thread->affinity_mask, conf->cpu_type);
...
int opusarium_thread_spawn(opusarium_thread_t *thread, opusarium_thread_conf_t *conf)
{
    ...
    if (conf->is_avoid_affinity)
        thread->affinity_mask = opusarium_cpu_mask_difference(cpu_mask, conf->affinity_mask, conf->cpu_type);
    else
        thread->affinity_mask = opusarium_cpu_mask_intersection(cpu_mask, conf->affinity_mask, conf->cpu_type);

    if (opusarium_unlikely(opusarium_cpu_mask_is_empty(thread->affinity_mask, conf->cpu_type))) {
        ret = -EINVAL;
        goto err_fail_state;
    }

    thread->numa_mask = opusarium_numa_conv_mask_from_cpu(thread->affinity_mask, conf->cpu_type);
    ...
    if ((thread->is_manual_stack = conf->is_manual_stack)) {
        ...
        thread->stack_ptr = opusarium_memory_getvirt(thread->stack_size,
                conf->default_stack_conf.stack_permission, thread->numa_mask,
                (OPUSARIUM_MEMORY_OPT_PAGEALIGN | OPUSARIUM_MEMORY_OPT_REQOSMEMORY), &ret);
    ...
    if (opusarium_unlikely(sched_setaffinity((int)thread->thread_envtid,
                    opusarium_cpu_get_mask_ref_as_envspec(thread->affinity_mask, conf->cpu_type),
                    opusarium_cpu_get_mask_size_as_envspec(thread->affinity_mask, conf->cpu_type)) < 0)) {
        ret = -ERANGE;
        goto err_kill_thread;
    }
