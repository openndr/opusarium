...
#include "common/opusarium_common_endian.h"
...
{
    return opusarium_endian_const_conv16(u16);
}
...
#define OPUSARIUM_ENDIAN_INTERNAL_COND(x, size, bval) \
    opusarium_builtin_const(x) ? \
    OPUSARIUM_ENDIAN_INTERNAL_CONST_##bval(x, size) : OPUSARIUM_ENDIAN_INTERNAL_RUNTIME_##bval(x, size)

#define OPUSARIUM_ENDIAN_INTERNAL_CONV(x, size, eval) \
    OPUSARIUM_ENDIAN_INTERNAL_COND(x, size, eval)

#define opusarium_endian_conv(x, size, endian) \
    OPUSARIUM_ENDIAN_INTERNAL_CONV(x, size, OPUSARIUM_ENDIAN_INTERNAL_##endian)

#define opusarium_endian_fastchk(endian) !(OPUSARIUM_ENDIAN_INTERNAL_##endian)
